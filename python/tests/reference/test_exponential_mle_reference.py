...
class ExponentialReferenceContracts(unittest.TestCase):
    ...
    def test_exp01_complete_sample_uses_independent_rational_golden(self) -> None:
        ...
        self.assertTrue(
            isclose(
                ...
                log_likelihood(2, "4.0", result.rate),
                rel_tol=1e-15,
                abs_tol=0.0,
            )
        )
        self.assertEqual(
            (result.observation_count, result.event_count, result.censored_count), (2, 2, 0)
        )

    def test_exp02_right_censoring_contributes_time_not_event(self) -> None:
        result = fit_exponential((ExactLifetime(Decimal("1")), RightCensoredLifetime(Decimal("3"))))
        ...
        self.assertTrue(
            isclose(
                result.log_likelihood,
                log_likelihood(1, "4", result.rate),
                rel_tol=1e-15,
                abs_tol=0.0,
            )
        )
        self.assertEqual(
            (result.observation_count, result.event_count, result.censored_count), (2, 1, 1)
        )
    ...
    def test_exp03_mixed_sample_has_closed_form_mle(self) -> None:
        ...
        self.assertTrue(
            isclose(
                result.log_likelihood,
                log_likelihood(2, "10", result.rate),
                rel_tol=1e-15,
                abs_tol=0.0,
            )
        )
        self.assertEqual(
            (result.observation_count, result.event_count, result.censored_count), (4, 2, 2)
        )
